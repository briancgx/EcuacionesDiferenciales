...
"""
eq = Eq(y(x).diff(x), 3 * x**2 * y(x) + x * exp(x**3))
""" 
determina la solución analítica
de la ecuación diferencial
"""
an_sol = dsolve(eq, hint='1st_linear', ics={y(0): 1})
""" 
imprime qué clase de ecuación diferencial es 
"""
print('Clase de ecuación diferencial: ', classify_ode(eq)[0])
""" 
...
plt.title('Ecuación diferencial lineal de primer orden')
plt.xlabel('x')
plt.ylabel('y(x)')
plt.legend()
plt.show()
